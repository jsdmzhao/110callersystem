...
jqWindow.prototype = {
			...
			if (!objWindow.jw.objqBtnMinimize) {
				// Create the button
		 		objWindow.jw.objqBtnMinimize = jQuery(document.createElement("div"))
		 						...
		 						.toggle(
								jqWindow.cbMinimizeHandler,jqWindow.cbMinimizeHandler
								);
	...
	build: function(cfgOptions) {
		...
		if (objElem.jw.hshOptions.contentURL) {
			 	alert($('.jwContentC').size());
			 $('.jwContentC').load(objElem.jw.hshOptions.contentURL,function(){
